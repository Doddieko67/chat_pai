...
import 'package:pai_app/presentation/screens/advanced_search_screen.dart';
...
class _TherapistListScreenState extends ConsumerState<TherapistListScreen> {
  ...
  void _openAdvancedSearch() async {
    ...
    if (result == true) {
      // Búsqueda realizada, actualizar lista con resultados
      final searchState = ref.read(therapistSearchProvider);
      if (searchState.therapists.isNotEmpty) {
        setState(() {
          therapists = searchState.therapists.map((twd) => twd.therapist).toList();
          filteredTherapists = List.from(therapists);
        });
        
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Encontrados ${searchState.therapists.length} terapeutas'),
  ...
  void _openMapView() async {
    ...
    try {
      ...
      final result = await Navigator.push<List<TherapistWithDistance>>(
        context,
        MaterialPageRoute(
      ...
      if (result != null) {
        // Actualizar la lista con los terapeutas del mapa
        setState(() {
          therapists = result.map((twd) => twd.therapist).toList();
          filteredTherapists = List.from(therapists);
        });
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error abriendo mapa: $e'),
